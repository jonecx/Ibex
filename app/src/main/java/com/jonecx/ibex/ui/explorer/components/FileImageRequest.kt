...
class DefaultFileImageRequestFactory @Inject constructor() : FileImageRequestFactory {
    override fun create(context: Context, fileItem: FileItem, fullSize: Boolean): ImageRequest {
        return ImageRequest.Builder(context)
            .data(fileItem.path)
            .size(if (fullSize) Size.ORIGINAL else Size(FileTypeUtils.THUMBNAIL_SIZE_PX, FileTypeUtils.THUMBNAIL_SIZE_PX))
            .crossfade(true)
            .apply {
                if (fullSize) {
                    parameters(Parameters.Builder().set(FULL_SIZE_KEY, FULL_SIZE_VALUE).build())
                }
                if (fileItem.fileType.isVideo) {
                    videoFrameMillis(FileTypeUtils.VIDEO_FRAME_TIME_MS)
                }
            }
            .build()
    }
...
fun Options.isFullSizeRequest(): Boolean =
    parameters.value<String>(DefaultFileImageRequestFactory.FULL_SIZE_KEY) ==
